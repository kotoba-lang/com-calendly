(ns calendly.main
  "Kotodama WASM entrypoint for the Calendly clean-room actor (L4) — Clojure port.
  ...
  facts (`calendly.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "calendly")
(def tier "L4")
...
(def entity-specs
  [{:entity "Workspace"  :plural "workspaces"  :id-prefix "calendly_wor"
    :fields [:name :ownerId :plan]                :required [:name :plan]
    :coerce {}                       :refs {}}
   {:entity "Document"   :plural "documents"   :id-prefix "calendly_doc"
    :fields [:workspaceId :title :contentRef :ownerId] :required [:title]
    :coerce {}                       :refs {:workspaceId "Workspace"}}
   {:entity "Folder"     :plural "folders"     :id-prefix "calendly_fol"
    :fields [:workspaceId :name :parentId]          :required [:name]
    :coerce {}                       :refs {:workspaceId "Workspace"}}
   {:entity "Comment"    :plural "comments"    :id-prefix "calendly_com"
    :fields [:documentId :authorId :body]            :required [:body]
    :coerce {}                       :refs {:documentId "Document"}}
   {:entity "Permission" :plural "permissions" :id-prefix "calendly_per"
    :fields [:resourceId :principalId :role]         :required [:role]
    :coerce {}                       :refs {}}
   {:entity "User"       :plural "users"       :id-prefix "calendly_use"
    :fields [:email :displayName :status]            :required [:email :displayName]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"calendly.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "calendly-compat" :tier tier :entities entities} 200])
...
(defn start! [] :calendly-compat/ready)
